...
pub const Database = struct {
    ...
    pub fn writeAll(self: *@This(), cards: []const Card) !void {
    ...
    pub fn readAll(self: *@This(), allocator: std.mem.Allocator) !std.ArrayList(Card) {
        const dir = std.fs.cwd();
        ...
        var cardsList: std.ArrayList(Card) = .empty;

        while (reader.interface.takeStruct(Card, .little)) |card| {
    ...
    pub fn initFile(self: *Database) !void {
        const dir = std.fs.cwd();
        self.mutex.lock();
    ...
    pub fn exists(self: *Database) bool {
        const dir = std.fs.cwd();
        self.mutex.lock();
